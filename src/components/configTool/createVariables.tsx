...
const App = (props) => {
  ...
  const[pvarDesc, setpvarDesc]=useState('');
  const[pvarComment, setpvarComment]=useState('');
    useEffect(() => {
        getLocations();
    }, []);
    const getLocations=()=>{
...
const createVariables = () => {
  ...
  axios({url:window.config.CG_CONFIG_CREATE_VARIABLES,method:'POST', 
    data: {
      "variableType": selectedVariableType,
      "variableName": variableName,
      "variableUnit": variableUnit,
      "linkedPointType": linkedPointType
    }
...
const addVariablesTree=()=>{
  // http://10.246.88.61:32769/energyanalytics/api/v1/add/variable
  // request:
  // {
   
  //   "pvarDesc": "Project Variable Description Example",
  //   "pvarComment": "This is a comment for the project variable, providing additional details."
  // }

  axios({url:window.config.CG_CONFIG_ADD_VARIABLE_TREE,method:'POST', 
    data: {
      "pvarDesc": pvarDesc,
      "pvarComment": pvarComment,
     
    }

  }).then(response=>{
  console.log('response: ',response);
  message.success('Successfully added variable');
...
createVariables();
    ...
    const  onChangeVariableTypeSelect=(e)=>{
      ...
      setvariableUnit(e);
          ...
          const onChangepvarDesc=(e)=>{
          setpvarDesc(e.target.value);
          }

          const onChangepvarComment=(e)=>{
setpvarComment(e.target.value)
          }
    return (
...
{checkedID1?<Input type='text' onChange={e => onChangeTextValues(e, 1)} value={pointUnitSelectionText} />:''}
              ...
              {/* <div className="textInputWrapper" style={{display:'flex'}}> 
                {['0','1','2','3','4'].map(item=> (item==checkedID) ?<Input type='text' value={item==0?regionValue:item==1?countryValue:item==2?cityValue:item==3?campusValue:item==4?buildingValue:''} onChange={item==0?setRegionValue:item==1?setCountryValue:item==2?setCityValue:item==3?setCampusValue:item==4?setBuildingValue:''} />:<Input type='text' disabled/>)}
                    ...
                    {props.selectedNodeFromTree && props.selectedNodeFromTree.project_id?<Button disabled={pvarComment==''|| pvarDesc==''} onClick={e=>addVariablesTree(e)}>Submit</Button>:<Button disabled={linkedPointType==''|| variableName==''|| variableUnit=='' || selectedVariableType==''} onClick={e=>submitCreateVariables(e)}>Submit</Button>
                    }
                </Layout>
